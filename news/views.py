...
def news_add(request):
    ...
    if request.method == 'POST':
        ...
        if news_title== "" or Cat== "" or Summary== "" or body== "":
        ...
        try:
            ...
            if str(image.content_type).startswith('image'):
                if image.size < 5000000:
                    ...
                    data= News(name=news_title, catname=newsname, summary=Summary, body=body, date=date, picname=filename, picurl=url, author=request.user, catid=newsid, ocatid=ocatid, show='0', tags=tags, rand=rand)
                    data.save()
                    ...
                    error= "Your File size is bigger than 5 MB"
                    return render(request, 'back/error.html', {'error': error})
            else:
                fs = FileSystemStorage()
                fs.delete(filename)
...
def show_all_news(request, name):
    ...
    return render(request, 'front/show_all_news.html',{'name':article_var, 'News': news, 'cat': cat, 'subcat': subcat, 'lastnews': lastnews, 'popularnews': popularnews, 'popularnews2':popularnews2, 'trending':trending, 'lastnews2':lastnews2, 'allnews': allnews })
